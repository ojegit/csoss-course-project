...
<font size=4>Course project: *Intrusion Detection System*<br/>Group: *LPR*<br/>Programming language(s): *Python (Part I compatible with Google Colab, Part II tested with 3.12)*<br/>
*Libraries:*<br/> 
    <li>Part I: [Colab Notebook](./colab_train_detector.ipynb) has extensive list of required libraries, but there's no need to install anything locally</li>
    <li>Part II: Tensorflow 2.18.0, Pandas 2.2.3, Numpy 2.0.2, Scikit-learn 1.6.1, Joblib 1.4.2</li></br> 
</font>
### Introduction
...
Network traffic was captured using PCAP files, and over 80 flow-level features were extracted using CICFlowMeter-V3, a tool that computes statistics such as packet size, duration, flow direction, and protocol behavior. Each flow is labeled as benign or as a specific attack type, making the dataset ideal for both binary and multi-class classification.

While the full dataset used in training the model is approximately 7 GB, for this project we focused on a manageable subset to demonstrate how an intrusion detection system (IDS) might work in practice using machine learning.

We have split this project into two parts. 

[Part I](#part-i-training-a-neural-network) covers everything about training a machine learning model which in this case is a Neural Network on CSE-CIC-IDS2018 dataset in Google Colab. This part is likely platform independent in that the code can be easily adapter to both *Windows*, *Linux* and even *OSX*. 
...
The code for training a neural network for the purpose of intrusion detection in Google Colab see [notebook](./colab_train_detector.ipynb). This model only detects binary labels.
...
### Demos
Demos *notebook_demo.mkv* and *cli_demo.mkv* are at ```./demos``` compressed.
